...
%% Statistical Analysis: Across-region similarity
fprintf('\n=== Across-Region Cluster Proportion Comparison ===\n');
...
fprintf('\nContingency Table (regions × clusters):\n');
fprintf('%-10s', 'Region');
for c = 1:5
...
fprintf('\n--- Chi-square test (permutation-based) ---\n');
...
fig_perm = figure('Position', [200, 200, 600, 400]);
histogram(chi2_perm, 50, 'Normalization', 'probability', 'FaceColor', [0.7 0.7 0.7]);
hold on;
xline(chi2_obs, 'r-', 'LineWidth', 2);
xlabel('Chi-square statistic', 'FontSize', 12);
ylabel('Probability', 'FontSize', 12);
title(sprintf('Permutation Distribution (p = %.4f)', p_perm), 'FontSize', 14);
...
for r1 = 1:n_regions
    for r2 = r1+1:n_regions
        % Extract 2-region contingency table
        pair_table = contingency_table([r1, r2], :);
        ...
        if size(pair_table, 2) > 0 && all(sum(pair_table, 2) > 0)
            ...
            fprintf('%s vs %s: chi2 = %.4f, p = %.4f\n', ...
                region_names_stat{r1}, region_names_stat{r2}, chi2_pair, p_pair);
        end
    end
end

% Calculate cluster proportions for correlation
proportions = zeros(n_regions, 5);
for br = 1:n_regions
    if ~isempty(results_all{br})
        region_total = results_all{br}.n_neurons;
        for c = 1:5
            proportions(br, c) = sum(results_all{br}.Clusters == c) / region_total;
        end
    end
end

%% Pearson correlation between brain regions
fprintf('\n=== Pearson Correlation Between Brain Regions ===\n');
...
correlation_matrix = zeros(n_regions, n_regions);
p_corr_matrix = ones(n_regions, n_regions);

for r1 = 1:n_regions
    for r2 = 1:n_regions
        if r1 == r2
            correlation_matrix(r1, r2) = 1.0;
            p_corr_matrix(r1, r2) = NaN;
        else
            [rho, p_val] = corr(proportions(r1, :)', proportions(r2, :)', 'Type', 'Pearson');
            correlation_matrix(r1, r2) = rho;
            p_corr_matrix(r1, r2) = p_val;

            fprintf('%s vs %s: r = %.4f, p = %.4f\n', ...
                region_names_stat{r1}, region_names_stat{r2}, rho, p_val);
        end
    end
end
...
fprintf('%-10s', 'Region');
for r = 1:n_regions
    fprintf('%10s', region_names_stat{r});
end
fprintf('\n');
for r1 = 1:n_regions
    fprintf('%-10s', region_names_stat{r1});
    for r2 = 1:n_regions
        fprintf('%10.4f', correlation_matrix(r1, r2));
    end
    fprintf('\n');
end

% Visualize Pearson correlation matrix
fig_corr = figure('Position', [300, 200, 600, 500]);
ax_corr = axes('Position', [0.15, 0.15, 0.65, 0.7]);

imagesc(ax_corr, correlation_matrix);
colormap(ax_corr, flipud(hot));
cb_corr = colorbar(ax_corr);
ylabel(cb_corr, 'Pearson r', 'FontSize', 12);
set(ax_corr, 'CLim', [0 1]);

% Set axis properties
set(ax_corr, 'XTick', 1:n_regions, 'XTickLabel', region_names_stat);
set(ax_corr, 'YTick', 1:n_regions, 'YTickLabel', region_names_stat);
set(ax_corr, 'FontSize', 12);
axis(ax_corr, 'square');
title('Pearson Correlation Between Regions', 'FontSize', 14, 'FontWeight', 'bold');
...
for r1 = 1:n_regions
    for r2 = 1:n_regions
        if r1 ~= r2
            ...
            val_str = sprintf('%.2f', correlation_matrix(r1, r2));

            % Add significance stars (from Pearson correlation p-values)
            p_val = p_corr_matrix(r1, r2);
            if p_val < 0.001
                sig_str = '***';
            elseif p_val < 0.01
                sig_str = '**';
            ...
            if correlation_matrix(r1, r2) > 0.5
                text_color = 'w';
            else
                text_color = 'k';
            end

            text(ax_corr, r2, r1, {val_str, sig_str}, ...
                'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
                'FontSize', 10, 'FontWeight', 'bold', 'Color', text_color);
        else
            % Diagonal
            text(ax_corr, r2, r1, '1.00', ...
                'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
...
annotation(fig_corr, 'textbox', [0.82, 0.70, 0.15, 0.15], ...
    'String', {'***  p<0.001', '**   p<0.01', '*    p<0.05', 'n.s. p≥0.05'}, ...
