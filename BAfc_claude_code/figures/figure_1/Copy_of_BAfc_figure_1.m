...
function export_figure1_to_excel(g)
    ...
    group_labels_bi = [ones(length(bi_LA_PN_clean), 1); 2*ones(length(bi_BA_PN_clean), 1); ...
        3*ones(length(bi_Astria_clean), 1); 4*ones(length(bi_CeA_clean), 1)];
    bi_combined = [bi_LA_PN_clean; bi_BA_PN_clean; bi_Astria_clean; bi_CeA_clean];
    ...
    sheet6{5,1} = 'BA PNs';
    sheet6{5,2} = length(bi_BA_PN_clean);
    sheet6{6,1} = 'AStria';
    sheet6{6,2} = length(bi_Astria_clean);
    sheet6{7,1} = 'CeA';
    sheet6{7,2} = length(bi_CeA_clean);
    sheet6{8,1} = '';

    sheet6{9,1} = 'Kruskal-Wallis Test: Burst index across regions';
    sheet6{10,1} = 'Chi-square statistic';
    sheet6{10,2} = tbl_kw_bi{2,5};
    sheet6{11,1} = 'Degrees of freedom';
    sheet6{11,2} = tbl_kw_bi{2,3};
    sheet6{12,1} = 'p-value';
    sheet6{12,2} = p_kw_bi;
    sheet6{12,3} = format_significance(p_kw_bi);
    sheet6{13,1} = '';
