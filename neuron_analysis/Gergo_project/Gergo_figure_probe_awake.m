% Gergo_figure_probe_awake

%% function Gergo_figure_4
...
t = tiledlayout(fig,12,8,'TileSpacing', 'compact', 'Padding', 'none');
...
% Load data from cell_metrics
g.mainFolder = 'C:\Users\dmagyar\Desktop\Gergo';
cd('C:\Users\dmagyar\Desktop\Gergo\AMY_shok_rest_run_Dani adatok')
load('cell_metrics_batch.mat')
ttlBatch1 = load('TTLsKS_Batch1.mat');
ttlBatch2 = load('TTLsKS_Batch2.mat');
ttlBatch3 = load('TTLsKS_Batch3.mat');
ttlBatch4 = load('TTLsKS_Batch4.mat');
g.cell_metrics = cell_metrics;
g.cell_metrics.general.shockTTL = cell(1,size(g.cell_metrics.cellID,2));
g.cell_metrics.general.shockTTL(find(g.cell_metrics.batchIDs==1)) = {ttlBatch1.shock_motor_rest};
g.cell_metrics.general.shockTTL(find(g.cell_metrics.batchIDs==2)) = {ttlBatch2.shock_motor_rest};
g.cell_metrics.general.shockTTL(find(g.cell_metrics.batchIDs==3)) = {ttlBatch3.shock_motor_rest};
g.cell_metrics.general.shockTTL(find(g.cell_metrics.batchIDs==4)) = {ttlBatch4.shock_motor_rest};
...
% Load data from Mclust
g.subfolders.LA = 'C:\Users\dmagyar\Desktop\Gergo\LA_awake';
...
g.subfolders.BA = 'C:\Users\dmagyar\Desktop\Gergo\BA_awake';
...
g.idx_LA(resp_spx<5) = 0;
g.idx_BA(resp_spx<5) = 0;
...
% Plot "big picture" heatmaps from all LA and BA neurons
% LA
nexttile(t,5,[4 2]);
imagesc(g.timeaxis,1:size(psth_spx_zscore_LA(order_LA,:),1),psth_spx_zscore_LA(order_LA,:));
clim(g.clim)
colormap(g.colors.Heatmap);
xline(0, '--k', 'LineWidth', g.xlinewidth/2, 'Alpha',1);
xlabel('Time (s)')
xticks([-g.pre_time 0 g.post_time]);
yticks([1 size(psth_spx_zscore_LA,1)])
set(gca, 'FontSize', g.fontSize2);
title('All LA neurons', 'FontSize', g.fontSize1)

% BA
nexttile(t,7,[4 2]);
imagesc(g.timeaxis,1:size(psth_spx_zscore_BA(order_BA,:),1),psth_spx_zscore_BA(order_BA,:));
clim(g.clim)
colormap(g.colors.Heatmap);
xline(0, '--k', 'LineWidth', g.xlinewidth/2, 'Alpha',1);
xlabel('Time (s)')
...
set(gca, 'FontSize', g.fontSize2);
title('All BA neurons', 'FontSize', g.fontSize1)

fn = fieldnames(g);                    % get all field names
fn(strcmp(fn,'cell_metrics')) = [];       % remove the one you want to keep from the list
g = rmfield(g, fn); 
clearvars -except g t 
...
ax = nexttile(t,83,[2 2]);
jitter_amount = 0.1;
jitter_LA = (rand(sum(g.idx_LA), 1) - 0.5) * jitter_amount;
jitter_BA = (rand(sum(g.idx_BA), 1) - 0.5) * jitter_amount;
psth_spx =  BAfc_psth_spx('cell_metrics', g.cell_metrics, 'ttl', 'shockTTL', 'pre_time', g.pre_time, 'post_time', g.post_time, 'bin_time', g.bin_time);
response_spikes = sum(psth_spx(:, round(g.pre_time/g.bin_time+15):round(g.pre_time/g.bin_time+45)),2);
...
%% cumulative diagram
ax = nexttile(t,87,[2 2]);
% allspikes_LA_ks= allspikes_LA(123:134);
% allspikes_BA_ks= allspikes_BA(123:134);

allspikes_LA_ks= postAP_spx_LA(postAP_spx_LA>0.015& postAP_spx_LA<0.045);
allspikes_BA_ks= postAP_spx_BA(postAP_spx_BA>0.015& postAP_spx_BA<0.045);
