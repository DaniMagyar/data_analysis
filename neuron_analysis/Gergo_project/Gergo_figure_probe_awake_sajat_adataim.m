% Gergo_figure_probe_awake_sajat adataim
...
psth_spx =  BAfc_psth_spx('cell_metrics', g.cell_metrics, 'ttl', 'triptest_shocks_only', 'pre_time', g.pre_time, 'post_time', g.post_time, 'bin_time', g.bin_time);
...
[psth_spx_bar, ~, ~, preAP_norm, postAP_norm] =  BAfc_psth_spx('cell_metrics', g.cell_metrics, 'ttl', 'triptest_shocks_only', 'pre_time', g.pre_time, 'post_time', g.post_time, 'bin_time', g.bin_time);
...
% Filter data
valid_LA = data_LA_all >= 0.005 & data_LA_all <= 0.25;
valid_BA = data_BA_all >= 0.005 & data_BA_all <= 0.25;
...
jitter_BA = (rand(sum(g.idx_BA), 1) - 0.5) * jitter_amount;
psth_spx =  BAfc_psth_spx('cell_metrics', g.cell_metrics, 'ttl', 'triptest_shocks_only', 'pre_time', g.pre_time, 'post_time', g.post_time, 'bin_time', g.bin_time);
response_spikes = sum(psth_spx(:, round(g.pre_time/g.bin_time+15):round(g.pre_time/g.bin_time+25)),2);
response_spikes = response_spikes./ cellfun(@numel,g.cell_metrics.general.triptest_shocks_only);
...
for ii = 1:size(g.cell_metrics.spikes.times,2)
    spikes = g.cell_metrics.spikes.times{ii};
    stimulus = g.cell_metrics.general.triptest_shocks_only{ii};
    for jj = 1:size(stimulus)
        spikes(spikes>stimulus(jj) & spikes<stimulus(jj)+excludesec) = [];
    end
    spikes = sort(spikes);
...
allspikes_LA_ks= postAP_spx_LA(postAP_spx_LA>0.015& postAP_spx_LA<0.025);
allspikes_BA_ks= postAP_spx_BA(postAP_spx_BA>0.015& postAP_spx_BA<0.025);
