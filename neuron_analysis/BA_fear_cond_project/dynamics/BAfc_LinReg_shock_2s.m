function [p] = BAfc_LinReg_shock_2s
...
for ii = 1:numel(cell_metrics.cellID)
    ...
    for kk = 1:size(psth_spx_zscore,1)
        aucShock_2sec(kk) = trapz(1:2000, psth_spx_zscore(kk,251:2250));
    end
    x = 1:20;
    y = aucShock_2sec;
    % b = bar(x, y);
...
cell_metrics.SlopeShock2s = p(:,1)';
cell_metrics.InterceptShock2s = p(:,2)';
