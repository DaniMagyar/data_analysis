...
for st = 1:size(g.ttl,2)
    for ii = 1:size(recordings,2)
        ...
        cell_metrics.LFP.(recordings{ii}).(g.ttl{st}).data = lfpAvg.data';
        cell_metrics.LFP.(recordings{ii}).(g.ttl{st}).timestamps = {lfpAvg.timestamps/lfpAvg.samplingRate};
        cell_metrics.LFP.(recordings{ii}).(g.ttl{st}).channelID = (1:64)';
        cell_metrics.LFP.(recordings{ii}).(g.ttl{st}).channelBR = channel_assignments.(recordings{ii}(1:5)).regions';
    end
end
