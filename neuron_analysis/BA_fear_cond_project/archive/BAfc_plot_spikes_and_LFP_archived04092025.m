...
cell_metrics = BAfc_load_neurons('recordings', g.recordings, 'ttl', g.ttl','LFP', true, 'notch', true,'twin', abs(g.twin));
LFP_LA = [];
LFP_BA = [];
...
[psth_spx, idx_1, ~] =          BAfc_find_response(g.recordings, g.ttl,    'exc', 0.5, 0.5, [0.013 0.027], 0.001, 'artefactLength',     0, 'psth_out', g.twin);
[~, idx_2, ~] =                 BAfc_find_response(g.recordings, g.ttl,    'exc', 0.5, 0.5, [0.028 0.039], 0.001, 'artefactLength',     0);
[~, idx_3, ~] =                 BAfc_find_response(g.recordings, g.ttl,    'exc', 0.5, 0.5, [0.040 0.092], 0.001, 'artefactLength',     0);
idx_all = unique([idx_1; idx_2; idx_3]);
% LA sounds
...
t = tiledlayout(2,2);
title(t, strjoin(string(g.recordings)))
