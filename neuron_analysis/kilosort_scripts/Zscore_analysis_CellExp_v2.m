function Zscore_analysis_CellExp_v2(varargin)
% Default params
prs =  inputParser;
addParameter(prs,'mainFolder','Z:\HajosLab\Dani\Magyar_Daniel\experiments\M2 drugs',@ischar) % Path of main folder. (e.g. 'C:\Users\dmagyar\Desktop\M2_shock_response')
...
addParameter(prs,'psth_bin',600,@isnumeric) % Binning window calculated from fs. (e.g. 1ms=30, 20ms=600(20x30), 50ms=1500(50x30))
addParameter(prs,'int',[-0.5 2],@isnumeric) % Plotting window [-pre post] in seconds.  
addParameter(prs,'Wcx_win',[-1 1],@isnumeric) % Time window [-pre post] for Wilcoxon in seconds. Two sides should be equal.
...
addParameter(prs,'average',1,@isnumeric) % Plot excited/inhibited group averages. Options: 1 or 0. 
