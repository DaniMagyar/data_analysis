...
addOptional(prs,'window',[-0.5 0.5],@(s)isnumeric(s)&isequal(length(s),2)) 
addOptional(prs, 'bin_time', 0.01,@isnumeric)
...
for nTT = 1:length(g.TTL)
    ...
    if size(ind_spikes,1) == 1
        line([ind_spikes; ind_spikes],[(ntt(nTT)-0.4)*ones(1,length(ind_spikes));...
            (ntt(nTT)+0.4)*ones(1,length(ind_spikes))],'Color','k','Linewidth',2.05); % itt eredetileg 0.05 volt
    elseif isempty(ind_spikes)
        ind_spikes = 1;
        line([ind_spikes; ind_spikes],[(ntt(nTT)-0.4)*ones(1,length(ind_spikes));...
            (ntt(nTT)+0.4)*ones(1,length(ind_spikes))],'Color','w','Linewidth',2.05);
    else
        line([ind_spikes ind_spikes]',[(ntt(nTT)-0.4)*ones(1,length(ind_spikes));...
            (ntt(nTT)+0.4)*ones(1,length(ind_spikes))],'Color','k','Linewidth',2.05);
    end
end
xlim([0 length(time)-1]);
xt = xticks;   
% xt2 = round(linspace(time(1),time(end),length(xt)),2); %Ez volt alapbol, de hibas olt ezzel az Y  es X tengely
% xticklabels(xt2);
% yt = yticks;      
% yticks(yt(end));
% yticklabels(ntt(end));
...
set(gca, 'xtick', xt(1):xt(end):xt(end)) % ezt en irtam, itt az ylim [0 nTT]
...
% ezt is en irtam bele, univerzalis betumeret
set(gca,'FontSize', 40)
