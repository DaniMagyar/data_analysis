...
function Zscore_analysis_CellExp_v3_Gergo(varargin)

% v3: exclude neurons that are labelled as 'junk' in CellExplorer.

% Default params
prs =  inputParser;
addParameter(prs,'mainFolder','C:\Users\dmagyar\Desktop\AMY_shok_rest_run',@ischar) % Path of main folder. (e.g. 'C:\Users\dmagyar\Desktop\M2_shock_response')
...
addParameter(prs,'psth_bin',30,@isnumeric) % Binning window calculated from fs. (e.g. 1ms=30, 20ms=600(20x30), 50ms=1500(50x30))
addParameter(prs,'int',[-0.1 0.1],@isnumeric) % Plotting window [-pre post] in seconds.  
addParameter(prs,'Wcx_win',[-0.1 0.1],@isnumeric) % Time window [-pre post] for Wilcoxon in seconds. Two sides should be equal.
...
for hh = cellIdx_selected
    ...
    for jj = 1:numel(TTL) %Each TTL is a a column. 
     ...
     for ll = 1:numel(TTL) %Each TTL is a a column. 
         ...
         if ~isempty(postAP{ll})
             postAP_norm_first{ll} = postAP{ll}(1)-TTL(ll);
         else 
             postAP_norm_first{ll} = [];
         end
     end

     % extracting first spike after ttl
     isEm =cellfun(@isempty,postAP_norm);
     postAP_norm_noEm = postAP_norm(~isEm);
     firstspikes = cellfun(@(getSpk)getSpk(1),postAP_norm_noEm); % getSpk -> this is short-writing a mini-function
     % extracting all spikes after ttl
     allspikes = vertcat(postAP_norm_noEm{:})'; 
     ...
     for tt = 1:numel(TTL) %Each TTL is a a column. 
         ...
         for oo = 1:(post_time/bin_time)
             ...
             postAP_bin_first(oo,tt) = sum(postAP_norm_first{tt}>=((oo-1)*bin_time) & postAP_norm_first{tt}<(oo*bin_time));
         end
     end  
     psth_spx_dani = vertcat(sum(preAP_bin,2), sum(postAP_bin,2));
     psth_spx_dani_first = vertcat(sum(preAP_bin,2), sum(postAP_bin_first,2)); 
     ...
     clear preAP_wcx postAP_wcx preAP_wcx_num postAP_wcx_num preAP_wcx_freq postAP_wcx_freq
            if g.norm == 1
                ...
                newpsth_first = zscore(psth_spx_dani_first);
            elseif g.norm == 0
                newpsth = psth_spx_dani;
                newpsth_first = psth_spx_dani_first;
            end
            firstspikeAll{find(cellIdx_selected==hh),:} = firstspikes; % firstspike matrix
            allspikesAll{find(cellIdx_selected==hh),:} = allspikes;
            PSTHall(find(cellIdx_selected==hh),:) = newpsth';
            PSTHall_first(find(cellIdx_selected==hh),:) = newpsth_first';
...
switch g.plotType
    ...
    case 'Lineplot'
        ...
        hold off
        subplot(2,2,3)
        plot (timeline,PSTHall_first) % plot individual neurons
        title('PSTHall_first')
        hold on
        plot(timeline, mean(PSTHall_first,1), 'Color', [.4 .4 .4], 'LineWidth', 3) % plot mean
        hold off
        subplot(2,2,2)
        bar(timeline,sum(PSTHall,1)) % plot individual neurons
        title('PSTHall')
        subplot(2,2,4)
        bar(timeline,sum(PSTHall_first,1)) % plot individual neurons
        title('PSTHall_first')
    ...
    case 'PCA'
        ...
        set(gca,'visible','off')
    case 'firstspikes'
        bar(sort([firstspikeAll{:}]))
    case 'allspikes'
        bar(sort([allspikesAll{:}]))
end
end
