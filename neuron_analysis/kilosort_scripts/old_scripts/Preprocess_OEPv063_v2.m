function Preprocess_OEPv063_v2(varargin)
...
prs =  inputParser;
addOptional(prs,'pLength',2.5,@isnumeric) 
...
incoming_data = zeros(info.Header.num_channels,length(info.Timestamps(firstEL:lastEL)),'int16');
% load data
loadBar = waitbar(0,'Loading channels...');
for ii = 1:info.Header.num_channels
    [next_channel] = info.Data.Data(1).mapped(channel_map(ii),firstEL:lastEL);
...
if g.removePeriod ~= [0 0]
    fid=fopen(['continuous_Preprocessed_' g.CHmap '_From' num2str(g.firstSec) '_To' num2str(g.lastSec) '_pLength' num2str(g.pLength) '_extracut' num2str(g.extracut) 'OverwritedFrom' num2str(g.removePeriod(1)) 'To' num2str(g.removePeriod(2)) '.dat'],'w+');
else 
    fid=fopen(['continuous_Preprocessed_' g.CHmap '_From' num2str(g.firstSec) '_To' num2str(g.lastSec) '_pLength' num2str(g.pLength) '_extracut' num2str(g.extracut) '.dat'],'w+');
end
