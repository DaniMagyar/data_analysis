function [group_averages] = BAfc_plot_pupil_changes_group(animalID, secPre, secPost, stim)
    % Load data
    %animalID = 'MD278';
    % secPre = 10;
    % secPost = 20;
    ...
    pupil_diameter_data = smoothdata(cell2mat(struct2cell(load(filename2))), 'movmean', 100); % 100fps video -> 100-500 smoothing (chatGTP)

    if exist([animalID '_data.mat'], 'file') == 2
        load([animalID '_data.mat'])
        disp(['Loading ' animalID '_data.mat'])
